...
//     const response = await fetch(`${BASE_URL}/viewIn/download/${filename}`, {
//       method: 'GET',
//       headers: {
...
/**
 * Returns a NORMAL image URL that can be used directly in:
 * <Image source={{ uri }} />
 *
 * No base64
 * No parsing
...
export const fetchBase64Image = async (
  ...
  try {
    ...
    // ✅ Just return the file URL with token support
    // Image component will fetch & render it normally
    return `${BASE_URL}/files/download/${filename}?token=${token}`;
  } catch (error) {
    console.error('Image URL build failed:', error);
    return FALLBACK_IMAGE;
  }
};
