...
export default function ChangeCommunityScreen() {
  ...
  // Track which community logos failed to load
  ...
  const fetchJoinedCommunities = useCallback(async () => {
    ...
    try {
      ...
      if (res?.success && Array.isArray(res.data)) {
        // Process communities with base64 image fetching
        const formatted = await Promise.all(
          res.data.map(async (c) => {
            let logoUri = FALLBACK_IMAGE;

            if (c.logo) {
              try {
                const uri = await fetchBase64Image(c.logo);
                if (uri) logoUri = uri;
              } catch (imgErr) {
                console.warn(`Failed to load logo for community ${c.id}:`, imgErr);
              }
            }
            ...
            return {
              ...
              description: c.description || t("no_description_available"),
              logo: logoUri,
              originalLogoKey: c.logo, // optional: keep for debugging
            };
          })
        );
                ...
                >
                  <View style={styles.communityRow}>
                    <View style={styles.iconCircle}>
                      <Image
                        source={{
                          uri: imageErrors[community.id]
                            ? FALLBACK_IMAGE
                            : (community.logo || FALLBACK_IMAGE),
                        }}
