...
export default function GetStarted() {
  ...
  useEffect(() => {
    async function prepare() {
      try {
        ...
        // Language should already be handled by i18next + detector / storage backend
        // But we can still read theme preference
        const savedTheme = await AsyncStorage.getItem('theme');
        if (savedTheme) {
          setIsDarkMode(savedTheme === 'dark');
        }
  ...
  const toggleLanguage = async () => {
    const newLang = i18n.language.startsWith('sw') ? 'en' : 'sw';
    await i18n.changeLanguage(newLang);
    // Optional: if you want to be explicit (though i18next usually handles storage)
    // await AsyncStorage.setItem('language', newLang);
  };
        ...
        >
          {[1, 2, 3].map((num) => (
            ...
            >
              ...
              <View style={themeStyles.overlayContent}>
                <View style={themeStyles.textWrapper}>
                  <Text style={themeStyles.heading}>{t(`onboarding.slide${num}.heading`)}</Text>
                  <Text style={themeStyles.subText}>{t(`onboarding.slide${num}.subText`)}</Text>
                </View>
              </View>
            </ImageBackground>
          ))}
...
// ────────────────────────────────────────────────
// Styles (minor tweak: increased gap between Skip & Language)
// ────────────────────────────────────────────────
...
const baseStyles = {
  ...
  topRight: {
    ...
    top: Platform.OS === 'android' ? (StatusBar.currentHeight || 0) + 10 : 30,
    right: 24,
    gap: 12,           // ← improved spacing between Skip & Change Language
  },
  topButton: {
    backgroundColor: 'rgba(255,255,255,0.2)',
    borderRadius: 20,
    paddingHorizontal: 14,   // slightly wider for better touch area
    paddingVertical: 7,
    flexDirection: 'row',
    alignItems: 'center',
  },
